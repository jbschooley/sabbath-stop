# Sabbath Stop — data layer

How the meetinghouse dataset is built, verified and served. The end-user
overview is in the [README](../README.md); the app design is in
[`APP_SPEC.md`](../APP_SPEC.md).

The data layer is a harvester that builds a static, tiled dataset of every
meetinghouse and unit, refreshed monthly by GitHub Actions and served from
GitHub Pages. The web app reads those static tiles.
...
| Buildings clusters listed that the harvest lacks | 2, both returned by identify with no ward units |
| Ward units clusters enumerated | 2,443 |
| Ward units clusters listed that the harvest lacks | 1, a seasonal ward identify never returns |
| Real gaps (sweep's fault) | **0** |

Measured on the first national run (CONUS, Alaska, Hawaii; 2026-09-20):
1,115 calls, 7,442 buildings, 15,507 units in 4.1 minutes. Clusters listed 11
buildings and 9 units the harvest lacked, every one classified upstream by the
identify re-probe. Zero real gaps.
...
Each candidate miss is re-probed with `identify` at its own cluster
coordinates before it counts as a gap. Two upstream inconsistencies look like
gaps but are not the sweep's fault: `identify` returning a building with no
ward units at all (the harvester drops those by design), and clusters listing
a unit that `identify` never returns anywhere (seasonal wards with no
meetinghouse do this). Both are reported as `upstream`; only an item that
`identify` returns right there and the sweep still failed to collect is a
`GAP`. See the docstring in `verify.py` for the current measured result.
...
# 1. Confirm the API answers. Always do this first.
...
Tile names use the floor of the tile's SW corner, with `n`/`s` and `e`/`w`
...
detour radius, and fetches only intersecting tiles. A cross-country route
...
all units and 100% of YSA wards while still looking like it works. The
harvester matches on the prefix, and `test_harvest.py` has a named regression
test for it.
...
This repo is public, and **nothing it talks to requires authentication**.
There is no key, token or credential anywhere in the code, and no `.env` to
forget about. The full list of hosts the harvester contacts:
...
The only thing to personalize is `USER_AGENT` in `harvest.py`, which carries
this repository's URL as its contact. If you fork, point it at your own.
...
Before pushing, confirm nothing snuck in:
...
- **Valhalla (FOSSGIS)** — `https://valhalla1.openstreetmap.de/route`.
  ...
  which is exactly what arrival-time matching needs. Valhalla shapes are
  encoded polylines at **precision 6**, not 5.
